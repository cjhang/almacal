...
# def gen_simulations_py3(obj, band, repeat, imagedir, outdir,
        # resolution, suffix):
def gen_simulations_py3(obj, band=None, repeat=1000, imagedir=None, outdir=None,
    ...
    if True:
        ...
        if os.path.isfile(imagefile):
            if os.path.isfile(os.path.join(obj_outdir, '{}_simulation'.format(obj))):
...
# def run_gen_simulations_py3(n_cores=4, objs=None, band='B7', repeat=5000, imagedir=None, outdir='./', 
        # resolution='0.3arcsec', suffix='image.fits'):
def run_gen_simulations_py3(n_cores=4, objs=None, **kwargs):
    ...
    def subprocess_gen_simulations(obj): gen_simulations_py3(obj, **kwargs)
    """
    if objs is None:
        objs = []
...
def calculate_correction(snr, simu_name, simu_boosting_file=None, flux_mode='aperture', 
...
def run_number_counts_p3(flist=None, detections_file=None, effective_area_file=None, band='B6',
    ...
    if True:
        snr_list, flux_boosting, comp_list, fake_rate_list = calculate_correction(
    ...
    for i in range(n_bootstrap):
        ...
        index_bootstrap = index_SMG + index_unknown_bootstrap
